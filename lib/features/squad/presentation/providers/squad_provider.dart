...
import 'package:flutter_clubapp/features/squad/domain/squad_repository.dart';
import 'package:flutter_clubapp/features/squad/data/supabase_squad_repository.dart';
...
class SquadProviderState {
  ...
  final List<SquadMemberMarker> members;
  ...
  SquadProviderState copyWith({
    ...
    String? squadPin,
    String? memberId,
    List<SquadMemberMarker>? members,
    List<SquadPin>? pins,
    String? errorMessage,
    bool? isMuted,
  }) {
...
class SquadMemberMarker {
  ...
  factory SquadMemberMarker.fromModel(
...
class SquadNotifier extends StateNotifier<SquadProviderState> {
  ...
  SquadNotifier(this._ref) : super(const SquadProviderState()) {
    // NIEUW: Luister naar profiel-updates. Als avatar/nickname verandert,
    // updaten we direct onze lokale weergave voor een snelle UI reactie.
    _ref.listen<UserProfileState>(userProfileProvider, (previous, next) {
      if (state.isInSquad && _myDbUserId != null) {
        _syncLocalProfileToSquad(next);
      }
    });
  }
  ...
  void _syncLocalProfileToSquad(UserProfileState profileState) {
    if (_myDbUserId == null) return;
    final updatedMembers = state.members.map((m) {
      if (m.odmemberId == _myDbUserId) {
        return m.copyWith(
          nickname: profileState.nickname ?? m.nickname,
  ...
  Future<SquadProviderState> createSquad() async {
    ...
    if (!authState.isAuthenticated) {
      final success = await _ref
          ...
          .signInAnonymously();
      if (success.status != AuthResultStatus.success) {
        state = state.copyWith(
          status: SquadConnectionStatus.error,
          errorMessage: 'Kan geen netwerkverbinding maken met de server.',
        );
        return state;
      }
    }
    ...
    if (!await checkLocationPermission() &&
      ...
      state = state.copyWith(
        status: SquadConnectionStatus.error,
        errorMessage: 'Locatietoegang is geweigerd.',
      );
      return state;
    }
    ...
    try {
      ...
      state = state.copyWith(
        status: SquadConnectionStatus.error,
        errorMessage:
            'Kan je locatie niet bepalen. Controleer of je GPS aan staat.',
      );
      return state;
    }
    ...
    try {
      final data = await _squadRepository.createSquadWithMember(
        profileState.nickname ?? 'Anonymous',
        userPosition,
      );
      ...
      _pinsSubscription = _squadRepository
          .subscribeToPins(squad.id)
          .listen(_onPinsUpdate);
      ...
      state = state.copyWith(
        status: SquadConnectionStatus.error,
        errorMessage: 'Er is iets misgegaan bij het aanmaken van de squad.',
      );
      return state;
    }
  }
  ...
  Future<SquadProviderState> joinSquad(String pin) async {
    ...
    try {
      ...
      state = state.copyWith(
        status: SquadConnectionStatus.error,
        errorMessage: 'Kan je locatie niet bepalen.',
      );
      return state;
    }
    ...
    try {
      ...
      if (squad == null) {
        state = state.copyWith(
          status: SquadConnectionStatus.error,
          errorMessage: 'Ongeldige squad PIN.',
        );
        return state;
      }
      ...
      state = state.copyWith(
        status: SquadConnectionStatus.error,
        errorMessage: 'Er is iets misgegaan bij het joinen van de squad.',
      );
      return state;
    }
  }
  ...
  Future<void> createPin(LatLng position, DateTime targetTime) async {
    ...
    await _squadRepository.createPin(
      state.squadId!,
      _myDbUserId!,
      position,
      targetTime,
  ...
  Future<void> _initWebRTC() async {
    ...
    _signalingChannel = Supabase.instance.client.channel(
      'squad_signaling_${state.squadId}',
    );
  ...
  void _handleSignalingMessage(Map<String, dynamic> rawData) async {
    ...
    if (rawData.containsKey('payload')) {
      ...
      if (innerPayload is Map) {
        if (innerPayload.containsKey('payload') &&
            innerPayload['payload'] is Map) {
        ...
        try {
          data = jsonDecode(innerPayload) as Map<String, dynamic>;
        } catch (_) {}
      }
    } else {
      data = rawData;
    ...
    try {
      switch (signalType) {
        ...
        case 'offer':
          ...
          await _peers[userId]!.setRemoteDescription(
        ...
        case 'ice-candidate':
          if (_peers.containsKey(userId)) {
            await _peers[userId]!.addCandidate(
              RTCIceCandidate(
                data['candidate'],
                data['sdpMid'],
                data['sdpMLineIndex'],
  ...
  void _updateMemberStateFromWebRTC(
    String userId, {
    LatLng? position,
    bool? isSpeaking,
  }) {
  ...
  bool _shouldSendPositionUpdate(LatLng currentPosition) {
    ...
    if (DateTime.now().difference(_lastSentTime!).inSeconds >=
        _positionUpdateFallbackSeconds) {
      return true;
    }
  ...
  void _checkOnlineStatus() {
    ...
    final updatedMembers = state.members.map((member) {
      final isOnline =
          DateTime.now().difference(member.lastUpdate).inMilliseconds <
          _offlineThresholdMs;
      return member.copyWith(
        isOnline: isOnline,
        isSpeaking: isOnline ? member.isSpeaking : false,
      );
    }).toList();
  ...
  void _onSquadMembersUpdate(List<models.SquadMember> modelMembers) {
    ...
    final displayMembers = modelMembers.map((m) {
      ...
      final isMe = m.userId == cId;

      return SquadMemberMarker(
        id: m.id,
        odmemberId: m.userId,
        nickname: isMe ? (profileState.nickname ?? m.nickname) : m.nickname,
        avatarUrl: isMe ? profileState.avatarUrl : m.avatarUrl,
        position: pos,
        lastUpdate: lastUpdate,
        isOnline: isOnline,
        isCurrentUser: isMe,
  ...
  int get _offlineThresholdMs {
    return _settingsService.trackingFrequency *
        _settingsService.offlineMultiplier *
        1000;
  }
  ...
  bool _listEquals(List<SquadMemberMarker> a, List<SquadMemberMarker> b) {
...
final squadProvider = StateNotifierProvider<SquadNotifier, SquadProviderState>((
  ref,
) {
