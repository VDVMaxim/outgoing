...
import 'package:flutter_clubapp/features/squad/presentation/widgets/squad_bottom_sheet.dart';
...
class _MapScreenState extends ConsumerState<MapScreen>
  ...
  Future<void> _openDetails(
    ...
    ref
        ...
        .logEvent(
          'viewed_place_details',
          parameters: {'place_id': markerPlace.id},
        );
    ...
    try {
      ...
      ShadToaster.of(context).show(
        ShadToast.destructive(
          ...
          description: Text(
            AppLocalizations.of(context)!.errorLoadingPlaceDetails,
          ),
        ),
      );
    }
    ...
    await showModalBottomSheet(
      ...
      builder: (context) => ClubBottomSheet(
        place: fullPlace!,
        userLocation: state.liveUserLocation ?? widget.userLocation,
      ),
    );
  ...
  Future<void> _handleTimePick(MapState state) async {
    final picked = await showTimePicker(
      context: context,
      initialTime: TimeOfDay.fromDateTime(state.pinTargetTime),
    );
    if (picked != null) {
  ...
  List<Marker> _buildMarkers(
    MapState mapState,
    SquadProviderState squadState,
    bool isDark,
  ) {
    final markers = <Marker>[];
    final activePlaces = mapState.cachedPlaces.values.where((place) {
      if (mapState.selectedFilters.isNotEmpty) {
        if (mapState.selectedFilters.contains('club') &&
            place.type != LocationType.club) {
          return false;
        }
        if (mapState.selectedFilters.contains('event') &&
            place.status != ClubStatus.event) {
          return false;
        }
      }
      if (mapState.currentZoom < 10.5) {
        if (!(place.hotnessScore >= 5 ||
    ...
    for (final place in activePlaces) {
      ...
      markers.add(
        buildPlaceMarker(
          ...
          isDark: isDark,
          isSelected: false,
          onTap: () => _openDetails(context, place, mapState),
        ),
      );
    }
    ...
    if (squadState.isInSquad) {
      ...
      final currentMember = squadState.members
      ...
      for (final pin in squadState.pins) {
        markers.add(
          buildSquadPinMarker(
            context: context,
            pin: pin,
            allMembers: squadState.members,
            currentUserId: currentMember?.odmemberId,
            isDark: isDark,
            onJoin: () => ref.read(squadProvider.notifier).joinPin(pin.id),
          ),
        );
      }
    }
    ...
    if (mapState.liveUserLocation != null && !squadState.isInSquad) {
      markers.add(
        buildUserLocationMarker(
          location: mapState.liveUserLocation!,
          currentHeading: mapState.currentHeading,
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<MapState>(mapProvider, (previous, next) {
      if (next.liveUserLocation != null &&
          next.liveUserLocation != previous?.liveUserLocation &&
          next.isFollowingUser) {
        _mapController.move(next.liveUserLocation!, next.currentZoom);
      }
      if (next.currentHeading != previous?.currentHeading &&
          next.isCompassMode) {
        _mapController.rotate(360 - next.currentHeading);
      }
      if (next.isCompassMode != previous?.isCompassMode &&
          !next.isCompassMode) {
    ...
    return Scaffold(
      body: !mapState.hasLocationPermission
          ? LocationFallbackView(
              isDark: currentIsDark,
              onAllowLocation: _promptForLocation,
            )
          : Stack(
              children: [
                FlutterMap(
                  ...
                  options: MapOptions(
                    ...
                    onMapReady: () {
                      ref
                          .read(mapProvider.notifier)
                          .fetchPlacesForCurrentBounds(_mapController.camera);
                    },
                ...
                if (!mapState.isPlacingPin)
                  MapSearchBar(
                    ...
                    onFilterTap: () =>
                        _showFilterSheet(context, currentIsDark, mapState),
                  ),
      ...
      floatingActionButton: MapControls(
        ...
        onMyLocationTap: () {
          ...
          if (mapState.liveUserLocation != null) {
            ...
            if (!mapState.isCompassMode && !mapState.isFollowingUser) {
              _mapController.rotate(0);
            }
          }
        },
