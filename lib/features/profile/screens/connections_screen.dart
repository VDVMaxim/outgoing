...
import 'package:flutter_clubapp/core/models/user_profile.dart';
...
class _ConnectionsScreenState extends ConsumerState<ConnectionsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _unfollow(String targetUserId) async {
    ...
    await ref.read(followControllerProvider.notifier).unfollowUser(targetUserId, currentUserId);
  }

  Future<void> _removeFollower(String followerId) async {
    final currentUserId = ref.read(userProfileServiceProvider).authUserId;
    if (currentUserId == null) return;
    await ref.read(followControllerProvider.notifier).removeFollower(followerId, currentUserId);
  }
      ...
      body: TabBarView(
        ...
        children: [
          followersAsync.when(
            ...
            error: (e, st) => Center(child: Text(AppLocalizations.of(context)!.errorLoadingFollowers)),
          ),
          followingAsync.when(
            data: (users) => _buildUserList(users, false, isDark),
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (e, st) => Center(child: Text(AppLocalizations.of(context)!.errorLoadingFollowing)),
          ),
        ],
      ),
    );
  ...
  Widget _buildUserList(List<UserProfile> users, bool isFollowersTab, bool isDark) {
    if (users.isEmpty) {
      return Center(
        child: Text(
          isFollowersTab ? AppLocalizations.of(context)!.noFollowers : AppLocalizations.of(context)!.followingNone,
          ...
          trailing: isMyProfile
              ? OutlinedButton(
                  onPressed: isProcessing ? null : () {
                    isFollowersTab ? _removeFollower(user.id) : _unfollow(user.id);
                  },
                  ...
                  child: Text(
                    isFollowersTab ? AppLocalizations.of(context)!.remove : AppLocalizations.of(context)!.unfollow,
