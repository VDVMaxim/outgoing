...
import '../../../core/services/vibe_points_service.dart';
import '../../../core/models/vibe_action.dart';
...
class VibeNotifier extends AsyncNotifier<VibeProfileState> {
  ...
  Future<VibeProfileState> _fetchVibeProfile() async {
    ...
    try {
      ...
      final actions = (actionsResponse as List).map((json) => VibeAction.fromJson(json)).toList();
  ...
  Future<void> submitVibeCheck({required String placeId, required bool isPositive}) async {
    final userId = _supabase.auth.currentUser?.id;
...
final vpDisplayProvider = Provider<VibeDisplayData?>((ref) {
  ...
  final nextVp = VibePointsService.getVpForNextLevel(state.currentLevel, state.totalVp);
