-- Comprehensive Seed Data for Flutter Club App
-- Run this AFTER schema.sql in your Supabase SQL Editor
-- NOTE: This will clear existing data and re-seed

-- Clear existing data (in correct order due to foreign keys)
DELETE FROM place_facilities;
...
-- Overpoort Area
INSERT INTO places (name, address, latitude, longitude, location_type, genre, status, event_name, organizer, promo, is_flash_promo_active, poi, crowd_level, wait_time, is_vereniging)
VALUES
...
INSERT INTO places (name, address, latitude, longitude, location_type, genre, status, poi, crowd_level)
VALUES
...
-- ============================================
-- JUNCTION TABLE LINKS (Tags & Facilities)
-- ============================================
...
DECLARE
    ...
    mirano_id UUID;
    upstairs_id UUID;
    bardofficier_id UUID;
    stadscafe_id UUID;
    
    -- Gent Other
    depint_id UUID;
    blaffendevis_id UUID;
    hofvanliere_id UUID;
    drank_id UUID;
    central_id UUID;
    deStoom_id UUID;
...
BEGIN
    ...
    SELECT id INTO salamander_id FROM places WHERE name = 'De Salamander';
    SELECT id INTO yucca_id FROM places WHERE name = 'Yucca';
    SELECT id INTO canard_id FROM places WHERE name = 'Canard Bizar';
    SELECT id INTO tweedekamer_id FROM places WHERE name = 'Tweedekamer';
    SELECT id INTO mirano_id FROM places WHERE name = 'Mirano';
    SELECT id INTO upstairs_id FROM places WHERE name = 'Upstairs';
    SELECT id INTO bardofficier_id FROM places WHERE name = 'Bar d''Officier';
    SELECT id INTO stadscafe_id FROM places WHERE name = 'Stadscafé';
    SELECT id INTO depint_id FROM places WHERE name = 'De Pint';
    SELECT id INTO blaffendevis_id FROM places WHERE name = 'Blaffende Vis';
    SELECT id INTO hofvanliere_id FROM places WHERE name = 'Hof van Liere';
    SELECT id INTO drank_id FROM places WHERE name = 'De Drank';
    SELECT id INTO central_id FROM places WHERE name = 'Café Central';
    SELECT id INTO deStoom_id FROM places WHERE name = 'De Stoom';
    SELECT id INTO deprof_id FROM places WHERE name = 'De Prof';
    SELECT id INTO cafebelge_id FROM places WHERE name = 'Café Belge';
    SELECT id INTO thehive_id FROM places WHERE name = 'The Hive';
    SELECT id INTO exterior_id FROM places WHERE name = 'Café Exterior';
    SELECT id INTO monarch_id FROM places WHERE name = 'Café Monarch';
    SELECT id INTO playing_id FROM places WHERE name = 'Café Playing';
    SELECT id INTO rompomp_id FROM places WHERE name = 'De Rompomp';
    SELECT id INTO mummie_id FROM places WHERE name = 'Café Mummie';
    ...
    SELECT id INTO various2_id FROM tags WHERE name = 'Dance';
    ...
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (clubctrl_id, dance_id),
      (clubctrl_id, techno_id),
      (clubctrl_id, underground_id),
      (clubctrl_id, flashpromo_id);
    
    -- Charlatan tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (charlatan_id, indie_id),
      (charlatan_id, rock_id),
      (charlatan_id, student_id),
      (charlatan_id, chill_id);
    ...
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (decap_id, techno_id),
      (decap_id, underground_id),
      (decap_id, electronic_id),
      (decap_id, vip_id);
    
    -- De Salamander tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (salamander_id, student_id),
      (salamander_id, gratisinko_id),
      (salamander_id, chill_id);
    
    -- Yucca tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (yucca_id, commercial_id),
      (yucca_id, studenten_id);
    
    -- Canard Bizar tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (canard_id, allround_id),
      (canard_id, cocktail_id);
    ...
    -- Mirano tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (mirano_id, mainstream_id),
      (mirano_id, dance_id);
    
    -- Upstairs tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (upstairs_id, chill_id),
      (upstairs_id, cocktail_id);
    
    -- Bar d'Officier tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (bardofficier_id, cocktail_id),
      (bardofficier_id, chill_id);
    
    -- Stadscafé tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (stadscafe_id, outdoor_id),
      (stadscafe_id, chill_id);
    
    -- De Pint tags
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (depint_id, pintjes_id),
      (depint_id, student_id);
    ...
    INSERT INTO place_tags (place_id, tag_id) VALUES
      (deprof_id, dance_id),
      (deprof_id, student_id),
      (deprof_id, gratisinko_id),
      (cafebelge_id, pintjes_id),
      (cafebelge_id, student_id),
      (thehive_id, dance_id),
      (thehive_id, student_id),
    ...
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (clubctrl_id, toilet_id),
      (clubctrl_id, vestiaire_id),
      (clubctrl_id, rookzone_id),
      (clubctrl_id, cashless_id);
    
    -- Charlatan facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (charlatan_id, toilet_id),
      (charlatan_id, rolstoel_id),
      (charlatan_id, buitenterras_id);
    
    -- Trio facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (trio_id, toilet_id),
      (trio_id, viplounge_id),
      (trio_id, vestiaire_id);
    
    -- Decap facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (decap_id, toilet_id),
      (decap_id, rookzone_id),
      (decap_id, vestiaire_id),
      (decap_id, viplounge_id);
    
    -- De Salamander facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (salamander_id, gratiswater_id),
      (salamander_id, lounge_id);
    
    -- Yucca facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (yucca_id, toilet_id),
      (yucca_id, vestiaire_id),
      (yucca_id, buitenterras_id);
    
    -- Canard Bizar facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (canard_id, toilet_id),
      (canard_id, buitenterras_id);
    
    -- Tweedekamer facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
    ...
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (mirano_id, toilet_id),
      (mirano_id, rolstoel_id),
      (mirano_id, vestiaire_id),
      (mirano_id, viplounge_id);
    
    -- Upstairs facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (upstairs_id, toilet_id),
      (upstairs_id, buitenterras_id);
    
    -- Bar d'Officier facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (bardofficier_id, toilet_id),
      (bardofficier_id, rolstoel_id);
    
    -- Stadscafé facilities
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (stadscafe_id, toilet_id),
      (stadscafe_id, buitenterras_id);
    ...
    INSERT INTO place_facilities (place_id, facility_id) VALUES
      (deprof_id, toilet_id),
      (deprof_id, airco_id),
      (cafebelge_id, toilet_id),
      (cafebelge_id, buitenterras_id),
      (thehive_id, toilet_id),
      (thehive_id, airco_id),
...
-- ============================================
-- SAMPLE VIBE CHECKS (for testing)
-- ============================================

INSERT INTO vibe_checks (place_id, crowd_level, energy, created_at)
SELECT 
    p.id, 
    p.crowd_level,
    CASE 
        WHEN p.crowd_level = 'busy' THEN 8
        WHEN p.crowd_level = 'medium' THEN 6
        WHEN p.crowd_level = 'vibes' THEN 7
        ELSE 5
    END,
    NOW() - (random() * interval '2 hours')
FROM places p
WHERE p.crowd_level IS NOT NULL;
